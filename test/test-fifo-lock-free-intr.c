...
typedef struct {
    fifo_head_lfi_t * head;
    const test_data_t *data_begin, *data_end;
    int64_t sum;
...
static void* ft_deleting(void * arg)
{
    ...
    while(true)
    {
        ...
        if(node != NULL)
        {
            ...
            s->sum += data->data;
            ++s->countok;
        } else if (atomic_load_explicit(& s->stop, memory_order_relaxed) && fifo_lfi_is_empty(s->head)) {
                return 0;
        } else {
            ++s->countmiss;
            // sched_yield();
...
/*
static bool check_fifo(const fifo_head_lfi_t * head)
{
    fifo_node_lfi_t *next = head->begin;
    fifo_node_lfi_t *node;
    int64_t sum = 0;
    uint32_t count = 0;
    test_data_t *data;
...
static void test_fifo_lfi_impl(const char * msg, fifo_head_lfi_t * head,
                                thread_func_t adding, thread_func_t deleting)
{
    ...
    sdel.head = head;
    sdel.data_begin = data_pool;
    sdel.data_end = data_pool + params.addthread * params.num_node_per_thread;
    ...
    for(i = 0; i < params.addthread; ++i)
    {
        if ( (ret = pthread_create(tadd + i, NULL, adding, sadd + i)) != 0)
        {
            LOG_WRITE_ERROR("pthread_create error: %s (%d)\n", FIFO_STRERROR(ret), ret);
            ck_assert_int_eq(ret, 0);
        }
...
START_TEST(test_fifo_lfi)
{
    fifo_head_lfi_t head;
    fifo_lfi_init(& head);
    test_fifo_lfi_impl(__func__, & head, ft_adding, ft_deleting);
...
{
    Suite *s = suite_create("fifo-lfi");
    TCase *tc;
    tc = tcase_create("fifo-lfi");
    tcase_add_test(tc, test_fifo_lfi);
